...
class Story_in_Sequence:
    ...
    #return the image content as np.array, and I import the mx.nd.array to transfer it to mxnet.nd.array            
	def show_you_story_data(self, story_id, show_image=True, show_sents=True):
		story = self.Stories[story_id]
		sent_ids = story['sent_ids']
		image_list=[]
		labels_list=[]
# 		lists=[image_list,labels_list]
		for i, sent_id in enumerate(sent_ids):
			img_id = self.Sents[sent_id]['img_id']
			img = self.Images[img_id]
			album_id = img['album_id']
			...
			img_file = osp.join(self.images_dir, split, img_id + '.jpg')
			img_content = self.read_img(img_file)
			img_content = np.asarray(img_content).transpose((2,0,1))
			image_list.append(img_content)
		if show_sents:
			for sent_id in sent_ids:
				sent = self.Sents[sent_id]
				labels_list.append(sent['text'])
	...
	def split_story(self):
		...
		return split_to_story_ids
    
	def re_dataset(self,split='train'):
		split_stories = self.split_story()
		print(len(split_stories[split]))
		counter = 0
		features = []
		labels = []
		for story in split_stories[split]:
# 			print('fuck')
			try:
# 				print('fuck you')
				feature,label = self.show_you_story_data(story)
